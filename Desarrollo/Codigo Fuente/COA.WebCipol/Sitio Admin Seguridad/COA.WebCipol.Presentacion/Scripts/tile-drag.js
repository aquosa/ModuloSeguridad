...
(function($) {
    ...
    $.TileDrag = function(element, options) {
        ...
        var dragStop = function (event) {
            var targetGroup, redirect;
            if (!mouseMoved) {
                // emulate default click behavior
                if ($draggingTile.children('a').length > 0) {
                	$redirec = $draggingTile.children('a');
                	if ($redirec.attr('href')) {
	                    if ($redirec.prop('target') === '_blank') {
	                        window.open($redirec.attr('href'));
	                    } else {
	                        window.location.href = $redirec.attr('href');
	                    }
                    }
                    else if ($redirec.attr('onclick')) {
                    	$redirec.click();
                    }
                }
